...
// Tree reduction idea on every 256 bits vector data
int avx2_version(const int* __restrict__ source, int* __restrict__ dest) {
    __builtin_assume_aligned(source, ALIGN);
    __builtin_assume_aligned(dest, ALIGN);

    // Initialize offset vector with zeros
    __m256i offset = _mm256_setzero_si256();

    // Process 8 integers at a time (256 bits)
    const int stride = AVX2_WIDTH_BITS / (sizeof(int) * CHAR_BIT);
    for (int i = 0; i < N; i += stride) {
        __m256i x = _mm256_load_si256((__m256i*)&source[i]);

        __m256i tmp0 = _mm256_bslli_epi128(x, 4); 
        
        __m256i tmp1 = _mm256_add_epi32(x, tmp0);

        __m256i tmp2 = _mm256_bslli_epi128(tmp1, 8);
        
        __m256i tmp3 = _mm256_add_epi32(tmp2, tmp1); 

        __m256i tmp4 = _mm256_permute2x128_si256(tmp3, tmp3, 0x08);
        __m256i out = _mm256_add_epi32(tmp4, tmp3);
        out = _mm256_add_epi32(out, offset);

        _mm256_store_si256((__m256i*)&dest[i], out);
        offset = _mm256_permutevar8x32_epi32(out, _mm256_set1_epi32(7));
    }
