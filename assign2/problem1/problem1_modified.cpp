...
int main(int argc, char *argv[]) {
  ...
  // HRTimer end = HR::now();
  // uint32_t duration= duration_cast<microseconds>(end - start).count();
  // cout << duration << "\n";
  return EXIT_SUCCESS;
}
...
void *thread_runner(void *th_args) {
  ...
  while (getline(input_file, line)) {
    // stopped true sharing by taking lock at last.
    total_lines++;
    ...
    while ((pos = line.find(delimiter)) != std::string::npos) {
      ...
      // stopped false sharing by putting each word_cound at different cache line
      // assuming each cache block of 64 byte and each word of 8 byte.
  ...
  tracker.total_lines_processed += total_lines;
  pthread_mutex_unlock(&line_count_mutex);

  pthread_mutex_lock(&tracker.word_count_mutex);
  tracker.total_words_processed += total_words;
