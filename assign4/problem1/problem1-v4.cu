...
int main()
{
    ...
    cudaCheckError(cudaHostAlloc(&h_in, SIZE_BYTES, cudaHostAllocDefault));
    cudaCheckError(cudaHostAlloc(&h_out, SIZE_BYTES, cudaHostAllocDefault));
    cudaCheckError(cudaHostAlloc(&h_out_1, SIZE_BYTES, cudaHostAllocDefault));
	...
	float t1, t2;
    cudaEvent_t start1, end1, end2, start2;
	...
	std::cout << "Kernel 2 time (ms) (without 2-way memcpy): " << t2 << "\n";
